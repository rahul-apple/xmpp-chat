//
//  TCChatConversationViewController.m
//  TChat
//
...
@interface TCChatConversationViewController ()
{
...
@implementation TCChatConversationViewController
...
-(void)loadHistory{
    ...
    NSString *receiver = [[_chatUserObject jidStr] stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"@%@",XAppDelegate.currentHost] withString:@""];
    
    _currentUser = [NSString stringWithFormat:@"%@@%@",XAppDelegate.username,XAppDelegate.currentHost];
    _buddy = [_chatUserObject jidStr];
    ...
    [XAppDelegate checkIfObjectExistsForEntityName:@"Chat" withPredicate:predicate inManagedObjectContext:XAppDelegate.managedObjectContext andCallback:^(id completionResponse) {
        if ([completionResponse isEqualToString:@"checkIfObjectExistsForEntityName:YES"]) {
            /*
             * Do nothing as message already loaded from server
             *
             */
            [HUD hide:YES];
        }
...
*/
#pragma mark - Chat message delegate methods
-(void)composingMessageReceived:(id)messageContent{
    
    [_composingStatusLabel setText:@"typing..."];
}
...
-(void)newMessageReceived:(NSDictionary *)messageContent{
   ...
   //TODO [XAppDelegate.receivedMessageSound play];
    ...
    //[self apiPostWithDictionary:messageContent];
    
    [_composingStatusLabel setText:@""];
    [self fetchItemsFromStore];
}
...
-(void)emoticonSelected:(id)selection{
    ...
    NSString *updatedSelection = [_textView text];
    updatedSelection = [updatedSelection stringByAppendingString:[NSString stringWithFormat:@" %@",selection]];
    [_textView setText:updatedSelection];
...
- (IBAction)emoticonAction:(UIButton *)sender {
  ...
  /*  if (emoticonInputViewShown) {
        ...
        [_emoticonSubView.view setNeedsDisplay];
        UIStoryboard *emoticonStoryboard = [UIStoryboard storyboardWithName:@"Emoticon-iPhone" bundle:nil];
        _emoticonSubView = [emoticonStoryboard instantiateInitialViewController];
        [_emoticonSubView.view setFrame:keyboardEmoticonRect];
        [_textView resignFirstResponder];
        [_textView setInputView:nil];
        _textView.inputView = _emoticonSubView.view;
        [_textView becomeFirstResponder];
